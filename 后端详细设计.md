...
- 认证
  - `POST /api/auth/register`
    ...
    - 响应：`{ code, message?, data: "注册成功" }`
  - `POST /api/auth/login`
    - 请求：`{ username, password }`
    - 响应：`{ code, message?, data: "Bearer <JWT_TOKEN>" }`
  - `POST /api/auth/logout`
    - 请求：无（需要Authorization头）
    - 响应：`{ code, message?, data: "登出成功" }`
  - `GET /api/user`
    - 请求：无（需要Authorization头）
    - 响应：`{ code, data:{ id, username, email, role, enabled, createdAt } }`
...
- 认证流程：
  - 登录时使用 `AuthService` 校验用户名/密码；成功后签发 `JWT Token(24h)`
  - 请求经过 `JwtAuthenticationFilter`，从 `Authorization` 头解析 Bearer Token，校验签名、过期、Redis有效令牌列表
- 核心组件：
  - `JwtTokenUtils`：生成/校验/解析 JWT；集成Redis令牌管理
  - `JwtAuthenticationFilter`：请求拦截器，验证JWT令牌并设置Security上下文
  - `AuthService`：处理用户注册、登录、密码哈希验证
  - `SecurityConfig`：配置Spring Security，集成JWT认证过滤器
- 令牌管理：`Redis key=user_tokens:<username>`，存储用户有效令牌集合，支持主动失效
- 密码安全：BCrypt哈希（强度12）+ 随机盐值，防止彩虹表攻击
...
```mermaid
graph TD
    A[用户访问系统] --> B{检查localStorage中的JWT令牌}
    B -->|无令牌| C[跳转登录页面]
    B -->|有令牌| D[验证令牌有效性]
    D --> E{令牌是否有效}
    E -->|无效| F[清除无效令牌]
...
sequenceDiagram
    ...
    F->>A: GET /api/user (携带Authorization头)
    A-->>F: 返回用户信息
    F-->>U: 跳转到编辑器页面
```
